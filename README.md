...
> [!Note]
> Control loops in Kubernetes can be deployed using Kubernetes Operator Framework. This is a native approach to orchestrate resources in Kubernetes, but the entry level is quit high and beyond the scope of our course. Additionally, operators may not be the best choice for managing higher-level abstractions where using dedicated applications can be more appropriate.
