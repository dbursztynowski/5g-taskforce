...
- Directory [_spiw-lab3_](./spiw-lab3): Lab3 guide for the SPIW course (pdf document)
...
Our goal is to illustrate how a service-level metric can be monitored in Open5GS core network and used in a simple control loop to scale selected network function (more specifically, a container performing the function of UPF in 5G core network). The lab focuses on the monitoring aspect and demonstrating a simple implementation of the scaling operation. The control loop will combine monitoring and scaling. You will design it within the project that follows the lab. However, we do not use Kubernetes operator framework for that. Instead, we use an external application containing all loop logic. We expect the lab and project will provide you with a foundation for designing and implementing your own, even more advanced control loops in Kubernetes.

> [!Note]
> Using Kubernetes Operator Framework is a native approach to orchestrate resources in Kubernetes. However, the entry level to this framework is quit high and so beyond the scope of our course. Additionally, Kubernetes operators may not be the best choice for managing higher-level abstractions with a complex logic. In such cases, it may be more appropriate to use dedicated applications having access to Kubernetes API, and our solution here follows this approach.

The control loop, driven by a selected service-level metric, will actuate on the resource level. It will vertically scale the UPF container in 5G core network (so without recreating the pod). Vertical scaling (in-place scaling) is beneficial for statefull services (while horizontal scaling may not suitable for stateful services). In Kubernetes, it is known as the so called In Place Pod Vertical Scaling (or In-Place Pod Resize). This feature was relatively new in Kubernetes (alpha status) during the preparation of the main part of this guide. It graduated to stable only in the release 1.35. We are currently using K3s v1.34 and apply relevant K3s installation procedure where InPlacePodVerticalScaling feature gate has to be explicitly enabled either during the installation or in a running cluster. Since Kubernetes 1.35, this feature gate has been enabled by default and should work out of the box (there's no need for manual configurations). One can give it a try together with the latest kube-prometheus release, but we have not checked those newer configurations so far in our environment.
