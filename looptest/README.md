...
In place pod scaling becomes increasingly better documented than a time ago with official description available [here](https://kubernetes.io/docs/tasks/configure-pod-container/resize-container-resources/). The example provided below derives from an earlier version of that page. You can try the official version if you wish.
...
> [!Note]
> Below, in the kubectl command, the full UPF pod name is used (`open5gs-upf-8444fdb48d-sv26l` in the example). You will have to replace it with the name of your instance. Writing the controller for the project, it will be useful to derive this name programatically. A simple method again relying on the kubectl command would be: (1) retrieve a list of pods from your 5G demo namespace: `kubectl get pods -n NAMESPACE` and (2) filter out the UPF pod name (string) prefixed `open5gs-upf`. We assume there is one UPF pod running in our network.
...
## 2.3 Retrieve the number of active UE sessions in Open5GS core network
