...
You already know how to monitor the number of UE sessions and how Open5GS functions can be scaled vertically without restarting the pod. Vertical scaling can be usefull for example in case of stateful functions, i.e., functions whose state can not be recreated easily after restarting the pod. This is the case with UPF function in Open5GS as a UPF container keeps the information about existing UE data plane sessions in RAM and not in persistent memory.
