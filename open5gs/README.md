...
This document describes how to install and run Open5GS 5G core network and UERANSIM RAN emulator using Helm charts from Gradiant on Raspberry Pi. The original (reference) version can be found [on this page](https://gradiant.github.io/5g-charts/open5gs-ueransim-gnb.html). It cannot be used directly in our case, as some adaptations are necessary to make the platform work on ARM64/Raspberry Pi and have a sufficient number of subscribers populated in the database. Nevertheless, you are encouraged to take a look at it. Hopefully you will find it helpful anyway.

The main results of this lab are a working instance of the Open5GS and UERANSIM platform and the ability to manage UEs in the network by attaching groups of UEs to the network and detaching such gropus from the network. Later on, in our project, we will be attaching/detaching UEs to trigger the UPF CPU scaling operation (UPF is the User Plane Function) in Open5GS core network. Also covered in this guide is traffic generation by the UEs although this skill is not mandatory to complete the lab (well, `ping` command can be used to verify that all works fine on the network level).

> [!NOTE]
> **If you have cloned this repository, it is ready to use and you can implement the entire platform (Open5GS and UERANSIM) fully customized to our needs. In this case, after reading this introduction, you can skip the section _Prepare Open5GS Helm chart_ and go directly to step [Deploy Open5GS](deploy-open5gs). This is the recommended approach to do the lab. However, if you are interested in the details of the modifications necessary to run the platform on Raspberry Pi, you can follow all the steps starting from the section [Prepare Open5GS Helm chart](prepare-open5gs-helm-chart).**

A simplified top-level view of the 5G environment we are going to work with is shown in the figure below. The network consists of two main parts: Open5GS, which acts as the 5G core network, and UERANSIM, which is the combined user equipment and RAN emulator. Open5GS exhibits standard 3GPP interfaces (compliant with Release 17 as of this writing) and both parts interwork using standard N2 and N3 interfaces (N1 interface is only partially implemented in UERANSIM - it basically covers the NAS layer while remaining layers are emulated by proprietary Radio Link Simulation protocol not compatible with the 3GPP stack). In the figure, we explicitly present only AMF, SMF and UPF and remaining functions of the core are represented in the figure in aggregated way (you will be able to find them in the cluster as Kubernetes deployments). As we will see later, each part of the platform (Open5GS, UERANSIM) can be created using Helm.

User equipment groups (UE groups) in the figure correspond to groups of terminals emulated by UERANSIM, where each group is implemented in UERANSIM by a distinct Kubernetes deployment. Creating or deleting UE groups is a simple mechanism to change the number of terminals attached to the network (active users). We will use this mechanism in our experiments. Each UE group (more precisely, the container that implements the group) can be logged in to execute commands that generate user traffic for selected terminals (UE) (ICMP/ping, HTTP/curl and Iperf are available out of the box). More detailed information about the internal structure of UERANSIM that is needed for doing the lab will be presented later in this document.
...
> [!NOTE]
> This modification is not mandated by the ARM architecture, but will extend the set of subscribers registered in the network. We will make use of it during the experiments.
> 
...
* enter the shell of respective `ueransim-gnb-uesX` container and change the permissions of the `nr-binder` executable (to be done once in a given `ueransim-gnb-uesX` Pod)
```
$ kubectl exec -it deployment/ueransim-gnb-ues -- /bin/bash
